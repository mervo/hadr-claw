...
### 2026-07-09 — static site pages (post-Tier-7 addition)

- Added `pages/architecture.html` (architecture & workflow) and
  `pages/marketing.html` (project overview) — hand-written static HTML,
  published to Pages by the heartbeat's stage-site step (`cp pages/*.html site/`).
  Not in the tier plan; user-requested docs/marketing pages. `index.html`
  remains the dashboard, untouched; the new pages are static and never
  regenerated by the run, so the "dev PRs don't diff dashboard/state" rule
  is unaffected.
