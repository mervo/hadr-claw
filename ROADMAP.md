...
| 2 | All feeds + unified schema + dedup | done | `--feeds usgs,gdacs,reliefweb` → one quake with multi-source badges; `scripts/check_dedup.py` | hadr/feeds/{gdacs,reliefweb}.py, hadr/dedupe.py, tests/fixtures/crossfeed/ |
| 3 | Memory & change detection | done | Run twice → second run quiet; day2 fixture → ESCALATED card; `scripts/check_memory.py` | hadr/memory.py, state/seen_events.json, tests/fixtures/day2/ |
