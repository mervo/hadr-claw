...
|---|------|--------|------|-----------|
| 0 | Docs foundation | in-progress | Read README/ROADMAP; `gh secret list` shows the key; PR raised | README.md, ROADMAP.md, CLAUDE.md, .env.example, implementation-notes.md |
| 1 | Slice one: USGS → dashboard (no LLM) | planned | `docker compose run --rm claw --feeds usgs` then `docker compose up dashboard` → http://localhost:8080; offline `--fixtures`; `uv run pytest` | pyproject.toml, Dockerfile, docker-compose.yml, hadr/{__main__,events,render}.py, hadr/feeds/usgs.py, tests/ |
